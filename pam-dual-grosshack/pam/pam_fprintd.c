 ...
 */
           ...
           if (r == 0)
             {
               ...
               if (has_recieved_sigusr1)
                 { pthread_mutex_lock(&data->mutex);
                   bool fc_success = data->fc_auth_success;
                   pthread_mutex_unlock(&data->mutex);
                   if (fc_success) {
                       if (debug)
                           pam_syslog(data->pamh, LOG_DEBUG, "Facial authentication succeeded");
                       return PAM_SUCCESS;}
                   else {
                     if (debug)
 ...
 static int
 do_mainloop_no_fprint (sd_bus *bus, verify_data *data) { 
   bool stop = false;
   while (!stop) {
     data->verify_ret = PAM_INCOMPLETE;
     pthread_mutex_lock(&data->mutex);
 ...
 prompt_pw (void *d)
 {
   ...
   char *pw;
   if (data->fp_avail) {
     pam_prompt (data->pamh, PAM_PROMPT_ECHO_OFF, &pw, "Auth by fingerprint, password or face: \n");
   } else {
     pam_prompt (data->pamh, PAM_PROMPT_ECHO_OFF, &pw, "Auth by password or face: \n");
   }
